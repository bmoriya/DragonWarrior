...
tantegel_courtyard = [
    ...
    courtyard_grass_line,
    courtyard_grass_line,
    [1] * 7 + [13] * 8 + [1] * 7 + [13] + [1, 1, 1] + [13, 14] + [13] * 9,
    [1] + [3] * 5 + [1, 13, 14, 13, 14, 14, 13, 14, 13, 1] + [3] * 5 + [1, 13, 1, 45, 1] + [13] * 11,
    [1] + [3] * 5 + [1] + [13] * 8 + [1] + [3] * 5 + [1, 13, 1, 2, 1] + [13] * 11,
    [1] + [3, 3, 1, 3, 3] + [1] * 4 + [3, 3] + [1] * 4 + [3, 3, 1] * 2 + [13, 13, 13, 14, 14] + [13] * 10,
    [1] + [3] * 20 + [1, 13, 14, 14, 14] + [13] * 11,
    [1] + [3] * 5 + [1] * 10 + [3] * 5 + [1] + [13] * 5 + [42] + [13] * 9,
    [1] * 5 + [3, 1, 3, 36] + [3] * 6 + [1, 1, 1, 5] + [1] * 5 + [3, 1, 1, 1] + [13] * 9,
    [1] + [3, 3, 3, 1, 3, 1, 7, 34] + [3] * 4 + [1, 3] + [1] + [3] * 11 + [1] + [13] * 9,
    [1] + [3, 41, 3, 3, 3, 1, 3, 38] + [3] * 6 + [1] + [3] * 11 + [1] + [13] * 9,
    [1] + [3, 3, 3, 1, 3, 1, 1, 1] + [3] * 4 + [1] * 12 + [3, 3, 1] + [13] * 9,
    [1] * 5 + [3, 1, 14, 14] + [3] * 4 + [14, 14, 1] + [3, 3, 1] * 4 + [13] * 9,
    [1] + [3, 3, 3, 1, 3, 1, 14, 14, 3, 3, 41, 3, 14, 14, 1] + [3, 3, 1] * 4 + [13] * 9,
    [1] + [3, 40, 3, 1, 3, 1, 14, 13] + [3] * 4 + [13, 14] + [1] + [3] * 11 + [1] + [13] * 9,
    [1] + [4, 3, 4, 5, 3, 1, 13, 42] + [3] * 4 + [13, 13] + [1] + [3] * 11 + [1] + [13] * 9,
    [1] + [3, 4, 3, 1, 3, 1, 13, 13] + [3] * 4 + [13, 13, 1] + [3, 3, 1] * 4 + [13] * 9,
    [1] + [4, 3, 4, 1, 3, 1, 13] + [3] * 6 + [13, 1] + [3, 3, 1] * 4 + [13] * 9,
    [1] * 5 + [3, 1, 13, 3] + [22] * 4 + [3, 13] + [1] * 10 + [3, 1, 1] + [13] * 9,
    [1] + [3] * 8 + [22, 8, 8, 22] + [3] * 6 + [3, 3, 1] + [3] * 5 + [1] + [13] * 9,
    [1] + [3] * 8 + [22, 8, 8, 22] + [3] * 6 + [3, 3, 1] + [8] * 5 + [1] + [13] * 9,
    [1] + [1, 1, 3, 3, 1, 1, 1, 3, 22, 22, 22, 22, 3, 1, 1] + [3] * 5 + [1] + [8] * 5 + [1] + [13] * 9,
    [1] + [3] * 5 + [3, 1] + [3] * 6 + [1, 39] + [3] * 5 + [1] + [3] * 5 + [1] + [13] * 9,
    [1] + [3] * 5 + [3, 1, 1] + [3] * 4 + [1] * 6 + [3, 3, 1, 44] + [3] * 4 + [1, 22] + [13] * 8,
    [1] + [3, 3, 1] + [3] * 4 + [1] + [3] * 4 + [1] + [3] * 7 + [1] * 6 + [1, 22] + [13] * 8,
    [1] + [3] * 5 + [3, 3, 1] + [3] * 4 + [1] + [3] * 6 + [3, 1] + [22] * 7 + [13] * 8,
    [1] + [3, 22, 22, 3, 3, 1, 45, 1] + [3] * 4 + [1, 3, 3] + [1] * 6 + [22] * 7 + [13] * 8,
    [1] + [22] * 4 + [3, 45, 3, 1] + [3] * 4 + [1] + [3, 3, 1] * 2 + [3, 1] + [22] * 7 + [13] * 8,
    [1] + [22] * 4 + [3] + [3, 3, 1, 1] * 2 + [3] * 5 + [2, 43, 1] + [22] * 7 + [13] * 8,
    [1] + [22] * 5 + [3, 3, 1, 39, 3, 3, 37, 1] + [3, 3, 1] * 2 + [3, 1] + [22] * 7 + [13] * 8,
    [1] * 10 + [3, 3] + [1] * 10 + [22] * 7 + [13] * 8,
    [22, 22] + [13] * 8 + [3, 3] + [13] * 8 + [22] * 9 + [6] + [13] * 7,
    courtyard_grass_line,
    courtyard_grass_line,
    courtyard_grass_line,
    courtyard_grass_line,
...
tantegel_courtyard = [[13] * 7 + row for row in tantegel_courtyard]
